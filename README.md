...
| method | api       | 기능       | 비고                                   |
|--------|:----------|:---------|--------------------------------------|
| GET    | /category | 카테고리 조회  | time_limit는 분단위, type P는 퍼센트, C는 카운트 |
| POST   | /start    | 문제 시작    | 호출할 때마다 랜덤으로 문제 세트를 만들어서 리턴          |
| GET    | /{id}     | 문제 단건 조회 |                                      |
| POST   | /move     | 문제 이동    | 리턴값 미정, 현재 진행상태와 체크한 답안만 업데이트 해준다.   |
...
```json
{
  ...
  "seq": 107,
  "category_id": 3,
  "question_set": "1,7,9,11,13,15",
  "progress_set": "0,0,0,0,0,0",
  "answer_set": "0,0,0,0,0,0",
  "correct_set": "0,0,0,0,0,0",
  "start_dt": "2022-07-20T19:32:28",
  "end_dt": null
}
```
...
- Request
```http request
GET http://3.37.139.180:9002/api/q/9
```
